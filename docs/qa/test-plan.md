...
| Layer       | Command                                                                      | Status                          | When                                      |
| ----------- | ---------------------------------------------------------------------------- | ------------------------------- | ----------------------------------------- |
| Unit        | `./gradlew :light-page:test`                                                 | ✅ Implemented                  | Every local commit / CI gate.             |
| Functional  | `just test-js`                                                               | ✅ Implemented                  | Every local commit / CI gate.             |
| Integration | `./gradlew :light-page:connectedDebugAndroidTest` (requires emulator/device) | ⚠️ Scaffolded (smoke test only) | Before merging, nightly, or QA milestone. |

## CI wiring

- `lint`, `test`, and `test-js` are now part of the `dev-check` recipe.
...
## Infrastructure changes made to enable tests

- `light-page/build.gradle.kts`: added `kotlin.test`, JUnit 5, MockK, and `kotlinx-coroutines-test` for unit tests; added `androidx.test` runner for instrumentation tests; enabled `BuildConfig`.
- `light-page/plugin/src/main/kotlin/com/thelightphone/plugin/LightSdkPlugin.kt`: test configurations (`testImplementation`, `androidTestImplementation`, etc.) and test source sets (`src/test`, `src/androidTest`) are now excluded from the production dependency allowlist and source-code validation, so test-only libraries and Android imports can be used without widening the APK allowlist.
